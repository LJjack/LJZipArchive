...
@interface LJZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end

@implementation LJZipArchive
{
    NSString *_path;
    NSString *_filename;
    zipFile _zip;
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(NSString *)password
                  error:(NSError **)error
               delegate:(id<LJZipArchiveDelegate>)delegate
        progressHandler:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total))progressHandler
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    // Begin opening
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"LJZipArchiveErrorDomain" code:-1 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (unzGoToFirstFile(zip) != UNZ_OK)
    {
        ...
        NSError *err = [NSError errorWithDomain:@"LJZipArchiveErrorDomain" code:-2 userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    do {
        @autoreleasepool {
            ...
            if (nil != err) {
                NSLog(@"[LJZipArchive] Error: %@", err.localizedDescription);
            }
                ...
                if (fp) {
                    ...
                    if (fileInfo.dosDate != 0) {
                        ...
                        if (attr) {
                            if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                                ...
                                NSLog(@"[LJZipArchive] Failed to set attributes - whilst setting modification date");
                            }
                        }
                    }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[LJZipArchive] Failed to set attributes - whilst setting permissions");
                        }
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[d objectForKey:@"modDate"], NSFileModificationDate, nil] ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
            NSLog(@"[LJZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
        }
        if (err) {
            NSLog(@"[LJZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
...
{
    BOOL success = NO;
    LJZipArchive *zipArchive = [[LJZipArchive alloc] initWithPath:path];
    if ([zipArchive open]) {
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[LJZipArchive] Attempting to close an archive which was never opened");
    zipClose(_zip, NULL);
    return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[LJZipArchive] MSDOS date masks don't add up");
